...
const createUser = TryCatch(async (req, res, next) => {
  ...
  const avatar = { publicid: "947384", url: "http://example.com/avatar.jpg" };
...
const myProfile = TryCatch(async (req, res) => {
  ...
  if (!user) {
    res.status(400).json({ error: "No Record found with this id" });
  }
  res.status(200).json({
    success: true,
    data: user,
...
const findUserById = TryCatch(async (userId) => {
